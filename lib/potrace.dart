...
class _Bitmap {
  ...
  _Bitmap.fromImg(img.Image image)
      : w = image.width,
        h = image.height,
...
class _Curve {
  ...
  _Curve(this.n)
      : tag = List.filled(n, null),
        c = List.filled(n * 3, null),
        vertex = List.filled(n, null, growable: false),
        alpha = List.filled(n, null),
        alpha0 = List.filled(n, null),
        beta = List.filled(n, null);
}
...
class _Quad {
  List<num?> data = List<num?>.filled(9, null);
  num? at(int x, int y) => data[x * 3 + y];
}

class _Opti {
  num pen = 0, t = 0, s = 0, alpha = 0;
  List<_DPoint?> c = List<_DPoint?>.filled(2, null);
}
...
class _ProcessPath {
  ...
  static _Point dorthInfty(_DPoint p0, _DPoint p2) {
    return _Point(sign(p2.x - p0.x).toInt(), -sign(p2.y - p0.y).toInt());
  }
  ...
  static void calcLon(_Path path) {
    ...
    List<num?> pivk = List.filled(n, null),
        nc = List.filled(n, null),
        ct = List.filled(4, null);
    path.lon = List.filled(n, null);
    ...
    for (i = n - 1; i! >= 0; i--) {
      ...
      while (true) {
        foundk = false;
        dir = ((3 + 3 * sign(pt[k!].x - pt[k1!].x) + sign(pt[k].y - pt[k1].y)) /
                2)
            .floor();
        ct[dir] = ct[dir]! + 1;
      ...
      if (foundk == false) {
        dk.x = sign(pt[k].x - pt[k1].x).toInt();
        dk.y = sign(pt[k].y - pt[k1].y).toInt();
  ...
  static void bestPolygon(_Path path) {
    ...
    List<num?> pen = List.filled(n + 1, null);
    List<int?> prev = List.filled(n + 1, null),
        clip0 = List.filled(n, null),
        clip1 = List.filled(n + 1, null),
        seg0 = List.filled(n + 1, null),
        seg1 = List.filled(n + 1, null);
    num thispen, best;
  ...
  static void adjustVertices(_Path path) {
    ...
    List<int?> po = path.po;
    List<_Point> pt = path.pt;
    List<_DPoint?> ctr = List.filled(m!, null, growable: false),
        dir = List.filled(m, null, growable: false);
    List<_Quad?> q = List.filled(m, null, growable: false);
    List<num?> v = List.filled(3, null, growable: false);
    int? i, j, k, l;
    _DPoint s = _DPoint();
    num d;
  ...
  void smooth(_Path path) {
    ...
    for (i = 0; i < m; i++) {
      ...
      if (denom != 0.0) {
        dd =
            dpara(curve.vertex[i]!, curve.vertex[j]!, curve.vertex[k]!) / denom;
        dd = dd.abs();
  ...
  static bool optiPenalty(_Path path, int? i, int j, _Opti res,
      num opttolerance, List<num?> convc, List<num?> areac) {
    int m = path.curve.n;
    int? k, k1, k2, conv, i1;
  ...
  void optiCurve(_Path path) {
    ...
    List<int?> pt = List.filled(m + 1, null, growable: false),
        len = List.filled(m + 1, null, growable: false),
        convc = List.filled(m, null, growable: false);
    List<num?> pen = List.filled(m + 1, null, growable: false),
        areac = List.filled(m + 1, null, growable: false);
    List<_Opti?> opt = List.filled(m + 1, null, growable: false);
    _Opti o = _Opti();
    _DPoint? p0;
    num? area, alpha;
    _Curve ocurve;
    ...
    om = len[m];
    ocurve = _Curve(om!);
    List<num?> s = List.filled(om, null, growable: false),
        t = List.filled(om, null, growable: false);
...
class _GetSVG {
  ...
  String run() {
    ...
    String svg = '<svg id="svg" version="1.1" width="' +
        w.toString() +
        '" height="' +
        h.toString() +
    ...
    svg += '<path d="';
    for (int i = 0; i < pathlist.length; i++) svg += path(pathlist[i].curve);
...
class _GetPath {
  ...
  void path(_Curve curve) {
    int n = curve.n;
    ret.moveTo(curve.c[(n - 1) * 3 + 2]!.x * (size as double),
        curve.c[(n - 1) * 3 + 2]!.y * (size as double));

    for (int i = 0; i < n; i++) {
      switch (curve.tag[i]) {
        case 'CURVE':
          ret.cubicTo(
              curve.c[i * 3 + 0]!.x * (size as double),
              curve.c[i * 3 + 0]!.y * (size as double),
              curve.c[i * 3 + 1]!.x * (size as double),
              curve.c[i * 3 + 1]!.y * (size as double),
              curve.c[i * 3 + 2]!.x * (size as double),
              curve.c[i * 3 + 2]!.y * (size as double));
          break;

        case 'CORNER':
          ret.lineTo(curve.c[i * 3 + 1]!.x * (size as double),
              curve.c[i * 3 + 1]!.y * (size as double));
          ret.lineTo(curve.c[i * 3 + 2]!.x * (size as double),
              curve.c[i * 3 + 2]!.y * (size as double));
          break;
      }
    }
  ...
  Path run() {
    for (int i = 0; i < pathlist.length; i++) path(pathlist[i].curve);
    return ret;
  }
}
